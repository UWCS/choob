...
public class ChoobDecoderTask extends ChoobTask {
	...
	static void updatePatterns() {
		triggerPattern = Pattern.compile("^(?:" + irc.getTriggerRegex() + ")",
	...
	public synchronized void run() {
		...
		if (event instanceof NickChange) {
			...
			if (nc.getNewNick().equals(irc.getNickname())) {
				updatePatterns();
				// Make sure the trigger checking code is up-to-date with the
				// current nickname.
		...
		if (event instanceof CommandEvent) {
			...
			if (mafind || mes instanceof PrivateMessage) {
				...
				if (matchAgainst.indexOf(' ') >= 0)
					matchAgainst = matchAgainst.substring(0, matchAgainst
							.indexOf(' '));
		...
		if ((event instanceof UserEvent)
				&& ((event instanceof FilterEvent) || ((mes != null) && mes
						.getFlags().containsKey("command")))) {
			try {
				if (1 == (Integer) ChoobDecoderTask.modules.plugin.callAPI(
						"UserTypeCheck", "Status", ((UserEvent) event)
								.getNick(), "bot"))
					ignoreTriggers = true;
		...
		if (!ignoreTriggers) {
			...
			if ((mes != null) && mes.getFlags().containsKey("command")) {
				...
				if (ma.matches()) {
					try {
						int ret = (Integer) modules.plugin.callAPI("Flood",
								"IsFlooding", mes.getNick(), 1500, 4);
						if (ret != 0) {
							if (ret == 1)
								irc
										.sendContextReply(mes,
												"You're flooding, ignored. Please wait at least 1.5s between your messages.");
							return;
						}
					} catch (ChoobNoSuchCallException e) {
					} // Ignore
					...
					ChoobTask task = modules.plugin.getPlugMan().commandTask(
							pluginName, commandName, mes);
					if (task != null)
						tasks.add(task);
				}
			}
