...
/**
 * Continually monitors the list of active callbacks, executing them as
 * required.
 */
			...
			synchronized (intervalList) {
				...
				while (tempIt.hasNext()) {
					...
					if (tempInterval.getTrigger() <= timeNow) {
						tempIt.remove();
						ChoobTask t = mods.plugin.doInterval(tempInterval
								.getPlugin(), tempInterval.getParameter());
						if (t != null) {
							try {
								ChoobThreadManager.queueTask(t);
							} catch (RejectedExecutionException e) {
								// Plugin is at concurreny limit. Requeue task
								// for later.
								tempInterval.setTrigger(tempInterval
										.getTrigger() + 1000);
								requeuedIntervals.add(tempInterval);
							} catch (Exception e) {
								System.err.println("Plugin "
										+ tempInterval.getPlugin()
										+ " got exception queuing task.");
								System.err.println(e);
								e.printStackTrace();
							}
						} else {
							System.err.println("Plugin manager for plugin "
									+ tempInterval.getPlugin()
									+ " returned a null doInterval ChoobTask.");
						}
