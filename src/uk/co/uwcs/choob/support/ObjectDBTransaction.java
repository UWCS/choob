 ...
 * perform various operations, such as adding, replacing and deleting items in a
 * single transaction (so either all, or none, of the operations occur).
 * 
...
{
	private static final int MAXOR = 50; // Max OR statements in a lumped

	// together objectDB query.
	private static final boolean USEMANYTABLES = true; // true for one table
	 ...
	 * 
	 * @throws ObjectDBDeadlockError
	 *             If the database has detected a deadlock, this exception is
	 *             thrown.
	 * @throws ObjectDBError
	 *             All other SQL-related exceptions are wrapped as
	 *             ObjectDBError.
	 */
	...
	/**
	 * Commits the transaction started by {@link #begin}, attempting to apply
	 * all operations to the database. It is possible that this will fail if,
	 * for example, the data being modified by this transaction has already been
	 * modified by another successful transaction. {@link #finish} must still be
	 * called afterwards.
	 * 
	 * @throws ObjectDBDeadlockError
	 *             If the database has detected a deadlock, this exception is
	 ...
	 * modifications made since then to be discarded. {@link #finish} must still
	 ...
	 * {@link #commit} has not been called, an implicit {@link #rollback} call
	 * is made first.
	 * 
	 * @throws ObjectDBDeadlockError
	 *             If the database has detected a deadlock, this exception is
	...
	private final ChoobError sqlErr(SQLException e) {
		...
		e.printStackTrace();
		return new ObjectDBError(
				"An SQL exception occurred while processing this operation.", e);
	}

	static final int TYPE_TINYINT = 0, TYPE_SMALLINT = 1, TYPE_INT = 2,
			TYPE_BIGINT = 3, TYPE_FLOAT = 4, TYPE_DOUBLE = 5, TYPE_TEXT = 6;

	// Unused as yet.
	static final int FLAG_UNSIGNED = 128;
	 ...
	 * 
	 * @param remove
	 *            the type of quotes you'll use in the query
	 * @param in
	 *            the String to quote
	...
	private final String getTableName(ObjectDBObject obj) {
		return "_objectdb_"
				+ obj.getClassName().toLowerCase().replaceAll("\\.", "_");
	}
	...
	private final void checkTable(ObjectDBClass cls) {
		try {
			...
			throw new ObjectDBError("Could not instanciate object of type: "
					+ cls.getName());
	...
	private final void checkTable(ObjectDBObject obj) {
		...
		try {
			...
			ResultSet results = stat.executeQuery("DESCRIBE `"
					+ clean("`", getTableName(obj)) + "`");
			if (results.first()) {
				do {
					// get the type name.
			...
			for (String field : fields) {
				...
				try {
					...
					if (type == null)
						throw new ObjectDBError("Unknown class type: "
								+ obj.getFieldType(field));
				} catch (NoSuchFieldException e) {
					throw new ObjectDBError("Unknown class type for field "
							+ field);
				}
			...
			while (clsIter.hasNext()) {
				...
				if (dbType != null && dbType == clsType) {
					...
					statements.add("DROP INDEX `"
							+ clean("`", thisName + "__index") + "`");
					statements.add("CHANGE `" + clean("`", thisName) + "` `"
							+ clean("`", thisName) + "` "
							+ dbReverseTypeMap.get(clsType)
							+ (clsType == TYPE_TEXT ? "" : " NOT NULL"));
					statements.add("ADD INDEX `"
							+ clean("`", thisName + "__index") + "` (`"
							+ clean("`", thisName) + "`"
							+ dbIndexLenMap.get(clsType) + ")");
					dbTypes.remove(thisName);
				} else {
					statements.add("ADD `" + clean("`", thisName) + "` "
							+ dbReverseTypeMap.get(clsType)
							+ (clsType == TYPE_TEXT ? "" : " NOT NULL"));
					statements.add("ADD INDEX `"
							+ clean("`", thisName + "__index") + "` (`"
	...
	private void generateTable(ObjectDBObject obj) {
		...
		try {
			...
			for (String field : fields) {
				...
				statements.add("ADD `" + clean("`", field) + "` "
						+ dbReverseTypeMap.get(type)
						+ (type == TYPE_TEXT ? "" : " NOT NULL"));
				statements.add("ADD INDEX `" + clean("`", field + "__index")
						+ "` (`" + clean("`", field) + "`"
						+ dbIndexLenMap.get(type) + ")");
			}
			...
			stat
					.executeUpdate("CREATE TABLE `"
							+ clean("`", getTableName(obj))
							+ "` (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY) Type=InnoDB");
			String initial = "ALTER TABLE `" + clean("`", getTableName(obj))
					+ "` ";
			for (String query : statements) {
				stat.executeUpdate(initial + query);
	 ...
	 * 
	 * @param storedClass
	 *            The class object (decendant of {@link Class} for Java,
	 *            {@link Function} for JavaScript) representing the type of
	 *            object desired to be retrieved.
	 * @param clause
	 *            The testricting part of the query, specifying which objects
	 ...
	 * 
	 * @param storedClass
	 *            The {@link ObjectDBClass} indicating the type of object
	...
	public final List<?> retrieve(final ObjectDBClass storedClass, String clause) {
		...
		ObjectDBClauseParser parser = new ObjectDBClauseParser("SELECT "
				+ select + " " + clause, storedClass.getName());
		parser.setUseMany(USEMANYTABLES);
		try {
			sqlQuery = parser.ODBExpr();
		} catch (ParseException e) {
			// TODO there's some public properties we can use to make a better
			// error message.
		...
		if (parser.getType() != ObjectDBClauseParser.TYPE_SELECT)
			throw new ObjectDBError("Clause string " + clause
					+ " was not a SELECT clause.");
		...
		if (USEMANYTABLES) {
			...
			try {
				...
				if (allObjects.first()) {
					...
					{
						Object newObject = storedClass.newInstance(); // This
						// will
						// be
						// set
						// immediately,
						...
						for (int i = 0; i < fields.length; i++) {
							...
							if (fieldType == String.class) {
								tempObject.setFieldValue(name, allObjects
										.getString(i + 1));
							} else if (fieldType == Integer.TYPE) {
								tempObject.setFieldValue(name, (int) allObjects
										.getLong(i + 1));
								...
								tempObject.setFieldValue(name,
										(float) allObjects.getDouble(i + 1));
					...
					} while (allObjects.next()); // Looping over blocks of
					// IDs
				}
				...
				throw new ObjectDBError(
						"Field that did exist now doesn't. Ooops?");
			} catch (InstantiationException e) {
				System.err.println("Error instantiating object of type "
						+ storedClass + ": " + e);
				throw new ObjectDBError("The object could not be instantiated.");
			} catch (IllegalAccessException e) {
				System.err.println("Access error instantiating object of type "
						+ storedClass + ": " + e);
				throw new ObjectDBError("The object could not be instantiated.");
			} catch (SQLException e) {
				throw sqlErr(e);
			...
			try {
				...
				String baseQuery = "SELECT ClassID, FieldName, FieldBigInt, FieldDouble, FieldString FROM ObjectStore LEFT JOIN ObjectStoreData ON ObjectStore.ObjectID = ObjectStoreData.ObjectID WHERE ClassName = '"
						+ storedClass.getName() + "' AND (";
				...
				// FIXME:{
				// FIXME: throw new ObjectDBError("Object of type " +
				// storedClass + " has no id field!");
				// FIXME:}
				...
				if (allObjects.first()) {
					...
					do // Loop over all objects
					{
						...
						ResultSet result = retrieveStat.executeQuery(query
								.toString());

						if (!result.first()) {
							// Ooops. To quote Sadiq: Um, yeah...
							// Actually, this is rather objects not existing in
							// ObjectStore when they, um, existed in
							// ObjectStore.
							throw new ObjectDBError(
									"Inconsistent database state: One or more objects of type "
											+ storedClass.getName()
											+ " in ObjectStore did not exist in ObjectStoreData.");
						}

						ObjectDBObject tempObject = null; // This will be set
						// immediately,
						// because 0 is not
						// a valid ID.
						int id = 0;
						...
						try {
							do // Loop over this block's results
							{
								try {
									...
									if (newId != id) {
										Object newObject = storedClass
												.newInstance();
										// Store the real object, then...
										objects
												.set(idMap.get(newId),
														newObject);
										// ...wrap the object so we are able to
										// use it!
									...
									if (name == null) {
										// XXX This is forbidden by the schema,
										// yet happens when the DB is broken.
										// Ie there's a null object.
										// Since it already got added, we're
										// safe, but the object will now
										// have all fields initialised to
										// default.
										continue;
									}
									...
									Type fieldType = fieldTypeCache.get(name);
									if (fieldType == null) {
										fieldType = tempObject
												.getFieldType(name);
										fieldTypeCache.put(name, fieldType);
									}

									if (fieldType == String.class) {
										tempObject.setFieldValue(name, result
												.getString(5));
									} else if (fieldType == Integer.TYPE) {
										tempObject.setFieldValue(name,
												(int) result.getLong(3));
									} else if (fieldType == Long.TYPE) {
										tempObject.setFieldValue(name, result
												.getLong(3));
									} else if (fieldType == Boolean.TYPE) {
							...
							System.err
									.println("Error instantiating object of type "
											+ storedClass + ": " + e);
							throw new ObjectDBError(
									"The object could not be instantiated.");
						} catch (IllegalAccessException e) {
							System.err
									.println("Access error instantiating object of type "
											+ storedClass + ": " + e);
							throw new ObjectDBError(
									"The object could not be instantiated.");
						}
	...
	public final List<Integer> retrieveInt(final ObjectDBClass storedClass,
			String clause) {
		String sqlQuery;
		...
		if (clause != null) {
			...
			try {
				...
				// TODO there's some public properties we can use to make a
			...
			sqlQuery = "SELECT ObjectStore.ClassID FROM ObjectStore WHERE ClassName = '"
					+ storedClass.getName() + "';";
		}
		...
		if (clause != null) {
			try {
				sqlQuery = ObjectDBClauseParser.getSQL(clause, storedClass
						.getName());
			} catch (ParseException e) {
				// TODO there's some public properties we can use to make a
				// better error message.
	 ...
	 * 
	 * @param strObj
	 *            The {@link ObjectDBObject} wrapping the real object to be
	 *            deleted.
	 */
	public final void delete(ObjectDBObject strObj) {
		...
		if (USEMANYTABLES) {
			...
			try {
				...
				delete = dbConn.prepareStatement("DELETE FROM `"
						+ clean("`", getTableName(strObj)) + "` WHERE id = ?");

				delete.setInt(1, id);

				if (delete.executeUpdate() == 0)
					throw new ObjectDBError(
							"Object for deletion does not exist.");
			} catch (SQLException e) {
				throw sqlErr(e);
			} finally {
				cleanUp(delete);
			...
			try {
				...
				PreparedStatement retrieveID = dbConn
						.prepareStatement("SELECT ObjectID FROM ObjectStore WHERE ClassName = ? AND ClassID = ?;");
				...
				delete = dbConn
						.prepareStatement("DELETE FROM ObjectStore WHERE ObjectID = ?");

				delete.setInt(1, objectID);

				deleteData = dbConn
						.prepareStatement("DELETE FROM ObjectStoreData WHERE ObjectID = ?");
	 ...
	 * 
	 * @param strObj
	 *            The object who's saved data is to be updated.
	 */
	 ...
	 * 
	 * @param strObj
	 *            The {@link ObjectDBObject} wrapping the real object who's
	 ...
	 * inside calls to {@link #begin} and {@link #commit}. If this method
	 * throws an exception, {@link #rollback} is called instead.
	 */
	public void run() {
	...
	private final void _store(ObjectDBObject strObj, boolean replace) {
		...
		if (USEMANYTABLES) {
			...
			try {
				...
				if (replace)
					stat = dbConn.prepareStatement("REPLACE INTO `"
							+ clean("`", getTableName(strObj)) + "` SET "
							+ values);
				else
					stat = dbConn.prepareStatement("INSERT INTO `"
							+ clean("`", getTableName(strObj)) + "` SET "
							+ values);
					...
					if (fieldName.equals("id")) {
						...
						try {
							...
							if (theType == java.lang.Integer.TYPE) {
								int theVal = ((Integer) strObj
										.getFieldValue(fieldName)).intValue();
								stat.setInt(c + offset, theVal);
							} else if (theType == java.lang.Long.TYPE) {
								long theVal = ((Long) strObj
										.getFieldValue(fieldName)).longValue();
								stat.setLong(c + offset, theVal);
							} else if (theType == java.lang.Boolean.TYPE) {
								boolean theVal = ((Boolean) strObj
										.getFieldValue(fieldName))
										.booleanValue();
								stat.setByte(c + offset, theVal ? (byte) 1
										: (byte) 0);
							} else if (theType == java.lang.Float.TYPE) {
								float theVal = ((Float) strObj
										.getFieldValue(fieldName)).floatValue();
								stat.setFloat(c + offset, theVal);
							} else if (theType == java.lang.Double.TYPE) {
								double theVal = ((Double) strObj
										.getFieldValue(fieldName))
										.doubleValue();
								stat.setDouble(c + offset, theVal);
							} else if (theType == String.class) {
								stat.setString(c + offset, (String) strObj
										.getFieldValue(fieldName));
							} else {
								// Urgh.
								throw new ObjectDBError(
										"Don't know type for variable "
												+ fieldName);
							}
						} catch (NoSuchFieldException e) {
				...
				if (id == 0) {
					...
					if (results.first())
						...
						throw new ObjectDBError(
								"Couldn't get the ID of the object which was saved...");
				}
			} catch (SQLException e) {
				throw sqlErr(e);
			} finally {
			...
			try {
				...
				if (id == 0) {
					stat = dbConn
							.prepareStatement("SELECT MAX(ClassID) FROM ObjectStore WHERE ClassName = ?;");
				...
				// There's no collision (more specifically, if there is one,
				// we're boned).
				if (objId == 0) {
					stat = dbConn
							.prepareStatement("INSERT INTO ObjectStore VALUES(NULL,?,?);");
				...
				if (replace)
					stat = dbConn
							.prepareStatement("REPLACE INTO ObjectStoreData VALUES(?,?,?,?,?);");
				else
					stat = dbConn
							.prepareStatement("INSERT INTO ObjectStoreData VALUES(?,?,?,?,?);");
					...
					if (!fieldName.equals("id")) {
						...
						try {
							...
							if (theType == java.lang.Integer.TYPE) {
								...
								stat.setString(2, fieldName);
								stat.setLong(3, 0); // XXX - parse these or not
								// parse these?
								stat.setDouble(4, 0);
								stat.setString(5, (String) strObj
										.getFieldValue(fieldName));
							} else
								foundType = false;
							...
							System.err
									.println("ERROR: Cast exception saving object class '"
											+ strObj.getClassName()
											+ "' property '"
											+ fieldName
											+ "', check ObjectDB wrapper implementation.");
	...
	private final Map<String, Object> permCache = new HashMap<String, Object>(); // Doesn't
	...
	private final void checkPermission(String objClass) {
		...
		if (cache == null) {
			AccessController.checkPermission(new ChoobPermission("objectdb."
					+ clsName));
