...
/* @bgen(jjtree) */implements ObjectDBClauseParserTreeConstants,
		ObjectDBClauseParserConstants {/* @bgen(jjtree) */
	protected JJTObjectDBClauseParserState jjtree = new JJTObjectDBClauseParserState();

	private int joins = 0;

	private List joinList = new ArrayList(); // List of properties to join

	// on.
	private Map nameMap = new HashMap(); // List of indices into joinList for

	// join names.
	private List classList = new ArrayList(); // List of classes used.

	private Map classMap = new HashMap(); // Map of alias name -> classList

	// offset.
	private Map fieldClass = new HashMap(); // Map of field name -> classList
	...
	public final static int HINT_NUMERIC = -1, HINT_UNKNOWN = 0,
			HINT_INTEGER = 1, HINT_FLOAT = 2, HINT_ID = 3, HINT_STRING = 4,
			HINT_BOOLEAN = 5;

	private int type = TYPE_UNKNOWN;

	public final static int TYPE_UNKNOWN = 0, TYPE_SELECT = 1, TYPE_UPDATE = 2,
			TYPE_DELETE = 3, TYPE_INSERT = 4;

	private boolean useMany = false; // XXX Only works with SELECT right
	...
	public void setUseMany(boolean useMany) {
		this.useMany = useMany;
	}
	...
	public static String getSQL(String clause, String className)
			throws ParseException {
		if (className == null)
			throw new IllegalArgumentException(
					"Null class name passed to getSQL...");
	...
	public String getTableJoin() {
		if (useMany) {
			...
			for (int i = 0; i < classList.size(); i++) {
				...
				joinText.append(getTableName(classN) + " s" + i);
			...
			for (int i = 0; i < joinList.size(); i++) {
				...
				joinText.append("ObjectStoreData o" + i + " ON s"
						+ fieldClass.get(fullName) + ".ObjectID = o" + i
						+ ".ObjectID AND o" + i + ".FieldName = \"" + fieldName
						+ "\"");
			}
			whereText.append(")");
	...
	public String getFieldName(String name) throws ParseException {
		...
		if (classID == null)
			throw new ParseException(
					"Attempted to use a class not in a WITH clause...");

		if (useMany) {
			realName = "s" + classID + "." + fieldName;
		} else {
	...
	public String parseExpression(SimpleNode expr, int hint)
			throws ParseException {
		...
		if (expr instanceof ObjectDBClauseNodeRoot) {
			...
			if (expr.jjtGetNumChildren() != 1)
				throw new ParseException(
						"Root node has a number of children other than 1. This should be impossible!");
			...
			for (int i = 0; i < expr.jjtGetNumChildren(); i++) {
				if (i != 0)
					buf.append(", ");
				buf.append(parseExpression((SimpleNode) expr.jjtGetChild(i),
						hint));
			}
			return buf.toString();
			...
			if (useMany)
				return prefix;
			else if (thingHint == HINT_INTEGER)
				...
				return prefix; // Special case; id fields are stored as the
			// full name anyway.
		}
		throw new ParseException("Could not parse node of type "
				+ expr.getClass().getSimpleName() + " with hint " + hint
				+ " into column name.");
	}

	public int getHint(SimpleNode expr1, SimpleNode expr2, int hint)
			throws ParseException {
		// Try to get hint from expr2, if that fails use expr1.
		return getHint(expr1, getHint(expr2, hint, true), false);
	}

	public int getHint(SimpleNode expr, int hint, boolean noResolve)
			throws ParseException {
		...
		case K_UPDATE:
			...
			if (s == null) {
				if (classList.size() > 0)
					...
					if (true)
						throw new ParseException(
								"No class specified for classless UPDATE");
				}
			}
			t = ParseUpdate(s);
			{
	...
	final public String ParseSelect(SimpleNode cols) throws ParseException {
		...
		label_1: while (true) {
			...
			case K_WITH:
				...
				if (cls == null) {
					if (true)
						throw new ParseException(
								"WITH clause used on a query which had no FROM class.");
				} else
					cls = string;
				break;
			case K_FROM:
				...
				if (cls == null)
					...
					if (true)
						throw new ParseException(
								"FROM clause used on a query which already had a FROM class.");
				}
				break;
			case K_SORT:
		...
		String realCols = (cols == null ? (useMany ? "s0.id" : "s0.ClassID")
				: parseExpression(cols, HINT_UNKNOWN));
		String tableJoin = getTableJoin(); // Also updates joinWhere.
		if (useMany) {
			{
				if (true)
					return "SELECT " + realCols + " FROM " + tableJoin
							+ " WHERE " + where + " AND " + joinWhere + sort
							+ limit;
			}
		} else {
			{
				if (true)
					return "SELECT " + realCols + " FROM " + tableJoin
							...
							+ " GROUP BY s0.ClassID" + sort + limit;
			}
		}
		throw new Error("Missing return statement in function");
	}
	...
	final public String ParseUpdate(String cls) throws ParseException {
		...
		for (int i = 0; i < cols.jjtGetNumChildren(); i++) {
			...
			inserts
					.append("INSERT IGNORE INTO ObjectStoreData SELECT ObjectID, \""
							+ col.getName()
							+ "\", NULL, NULL, NULL FROM ObjectStore WHERE ClassName = \""
							+ cls + "\";\n");

			// Parse once for strings
			int hint = getHint((SimpleNode) col.jjtGetChild(0), HINT_UNKNOWN,
					false);
			String result = parseExpression((SimpleNode) col.jjtGetChild(0),
					hint);
	...
	final public String ParseInsert() throws ParseException {
		...
		case K_INTO:
			...
			if (cls == null)
				...
				if (true)
					throw new ParseException(
							"INTO clause used on a query which already had an INTO class.");
			}
			break;
		default:
			jj_la1[6] = jj_gen;
		...
		for (int i = 0; i < cols.jjtGetNumChildren(); i++) {
			...
			if (col.getName().equalsIgnoreCase("id"))
				idClause = parseExpression((SimpleNode) col.jjtGetChild(0),
						HINT_UNKNOWN);
		}

		StringBuffer query = new StringBuffer(
				"INSERT INTO ObjectStore VALUES(DEFAULT, '" + cls + "', "
						+ idClause + ");\n");
		for (int i = 0; i < cols.jjtGetNumChildren(); i++) {
			SimpleNode col = (SimpleNode) cols.jjtGetChild(i);
			...
			String value = parseExpression((SimpleNode) col.jjtGetChild(0),
					HINT_UNKNOWN);
			query
					.append("INSERT INTO ObjectStoreData VALUES(LAST_INSERT_ID(), \""
							+ col.getName()
							+ "\", "
							+ value
							+ ", "
	...
	final public String ParseDelete() throws ParseException {
		...
		{
			if (true)
				return "DELETE FROM s0 USING " + tableJoin + " WHERE " + where
						+ " AND " + joinWhere;
		}
		throw new Error("Missing return statement in function");
	}
	...
	final public String BoolExpression() throws ParseException {
		...
		if (expr2 == null) {
			...
			// Attempt to mangle together a further hint, for example narrowing
			// HINT_NUMERIC.
			hint = getHint(expr1, expr2, hint);
			{
				if (true)
					return parseExpression(expr1, hint) + " " + op + " "
							+ parseExpression(expr2, hint);
			}
		}
		throw new Error("Missing return statement in function");
	}
	...
	final public void Function(String name, int params) throws ParseException {
		...
		ObjectDBClauseNodeFunction jjtn000 = new ObjectDBClauseNodeFunction(
				JJTFUNCTION);
		boolean jjtc000 = true;
		jjtree.openNodeScope(jjtn000);
		SimpleNode n;
		try {
			...
			if (n.jjtGetNumChildren() != params) {
				if (true)
					throw new ParseException("Wrong number of parameters for "
							+ name + "!");
			}
	...
	final public void Constant() throws ParseException {
		...
		ObjectDBClauseNodeConstant jjtn000 = new ObjectDBClauseNodeConstant(
				JJTCONSTANT);
		boolean jjtc000 = true;
		jjtree.openNodeScope(jjtn000);
		Token value;
	...
	final public void Field(int hint) throws ParseException {
		...
		try {
			...
			case IDNAME:
				...
				break;
			case QIDNAME:
				name = jj_consume_token(QIDNAME);
				jjtree.closeNodeScope(jjtn000, true);
				jjtc000 = false;
				jjtn000.setName(name.image
						.substring(1, name.image.length() - 1));
				jjtn000.setHint(HINT_ID);
				break;
			case FIELDNAME:
				...
				break;
			case QFIELDNAME:
				name = jj_consume_token(QFIELDNAME);
				jjtree.closeNodeScope(jjtn000, true);
				jjtc000 = false;
				jjtn000.setName(name.image
						.substring(1, name.image.length() - 1));
	...
	final public String ClassName() throws ParseException {
		...
		case FIELDNAME:
			...
			break;
		case QFIELDNAME:
			t = jj_consume_token(QFIELDNAME);
			cls = t.image.substring(1, t.image.length() - 1);
			break;
		default:
			jj_la1[21] = jj_gen;
			jj_consume_token(-1);
		...
		if (classMap.get(name.toLowerCase()) == null) {
			classMap.put(name.toLowerCase(), classList.size());
			classList.add(cls);
		}
	...
	final public SimpleNode ExpressionList() throws ParseException {
		...
		ObjectDBClauseNodeExpressionList jjtn000 = new ObjectDBClauseNodeExpressionList(
				JJTEXPRESSIONLIST);
		boolean jjtc000 = true;
		jjtree.openNodeScope(jjtn000);
		try {
	...
	final public void FullFieldSet() throws ParseException {
		...
		ObjectDBClauseNodeFieldSet jjtn000 = new ObjectDBClauseNodeFieldSet(
				JJTFIELDSET);
		boolean jjtc000 = true;
		jjtree.openNodeScope(jjtn000);
		List list = new ArrayList();
	...
	final public SimpleNode FullFieldSetList() throws ParseException {
		...
		ObjectDBClauseNodeFieldSetList jjtn000 = new ObjectDBClauseNodeFieldSetList(
				JJTFIELDSETLIST);
		boolean jjtc000 = true;
		jjtree.openNodeScope(jjtn000);
		List list = new ArrayList();
	...
	final public String ParseSort() throws ParseException {
		...
		case FIELDNAME:
			...
			t = jj_consume_token(FIELDNAME);
			String suffix = useMany ? "" : isInt ? ".FieldBigInt"
					: ".FieldString";
			{
				if (true)
					return "ORDER BY " + getFieldName(t.image) + suffix + " "
							+ order;
			}
			break;
		case K_RANDOM:
	...
	private static void jj_la1_0() {
		jj_la1_0 = new int[] { 0x8000, 0x0, 0x1e0000, 0x3e00, 0x3e00, 0x200,
				0x10000, 0x2000, 0x200, 0x2000000, 0x1000000, 0x80a00000, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x80200000, 0x0, 0x0, 0x0, 0x40000000,
				0x0, 0x40000000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xc000000,
				0xc000000, 0x10000000, 0x3c000000, 0x400000, };
	}

	private static void jj_la1_1() {
		jj_la1_1 = new int[] { 0x0, 0x300000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x1ffc007, 0x1f8, 0x1f8, 0x600, 0x600, 0x1800,
				0x1800, 0x1ffc007, 0x1c00000, 0x3c0000, 0x300000, 0x0,
				0x300000, 0x300000, 0x2000, 0x2000, 0x3c0000, 0x2000, 0x300000,
				0x2000, 0x0, 0x0, 0x0, 0x100000, 0x2000, };
	}
