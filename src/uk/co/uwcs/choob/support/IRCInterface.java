...
public final class IRCInterface {
	...
	public final static int MAX_MESSAGES = 3; // Max messages before /msg is
	...
	/**
	 * Similar to the sendContextReply function, except sends an action instead
	 * of a message.
	 * 
	 * @param context
	 *            A context object to target the reply at.
	 * @param message
	 *            A String of the /me you want to send.
	 */
	public void sendContextAction(ContextEvent context, String message) {
		...
		if (context instanceof PrivateEvent
				|| mods.pc.isProtected(context.getContext()))
	 ...
	 * 
	 * @param messages
	 *            The strings to break apart.
	 * @param prefix
	 *            A length to ensure space for in the cut strings.
	...
	public List<String> cutString(String message, int prefix) {
		...
		while (true) {
			if (message.length() - pos > max_length) {
				...
				if (spacePos == -1
						|| pos + max_length - spacePos > MAX_MESSAGE_TRUNCATION) {
	...
	/**
	 * Sends an in-context response to an event. Automatically decides between
	 * replying in the channel the event was triggered from, and private
	 * messaging the response. This is the best function to call, unless you
	 * never want the reply to appear in the channel.
	 * 
	 * @param context
	 *            A context to reply in.
	 * @param messages
	 *            A Strings[] containing the message you want to send.
	 * @param prefix
	 *            If in a channel, should the reply be prefixed with
	 *            "&lt;Nick&gt;: "?
	 */
	public void sendContextReply(ContextEvent context, String[] messages,
			boolean prefix) {
		if (!(context instanceof UserEvent))
			throw new IllegalArgumentException("ConextEvent " + context
					+ " passed to sendContextReply was not a contextEvent!");
	 ...
	 *            A context to reply in.
	 * @param messages
	 *            A list of Strings containing the message you want to send.
	 * @param prefix
	 *            If in a channel, should the reply be prefixed with
	 *            "&lt;Nick&gt;: "?
	 */
	public void sendContextReply(ContextEvent context, List<String> messages,
			boolean prefix) {
		if (!(context instanceof UserEvent))
			throw new IllegalArgumentException("ConextEvent " + context
					+ " passed to sendContextReply was not a contextEvent!");
	 ...
	 *            A context to reply in.
	 * @param message
	 *            A String containing the message you want to send.
	 * @param prefix
	 *            If in a channel, should the reply be prefixed with
	 *            "&lt;Nick&gt;: "?
	 */
	...
	private void privateSendContextMessage(ContextEvent context,
			List<String> lines, boolean prefix) {
		String nick = ((UserEvent) context).getNick();
		...
		if (context instanceof PrivateEvent
			...
			privateSendMessage(
					context.getContext(),
					nick
							+ ": Sorry, the output is too long! Private messaging it to you!");
			target = nick;
			thePrefix = "";
		} else {
			// Send to channel
	...
	/**
	 * Sends a message to the target you specify, may be a #channel. It is
	 * better to use sendContextReply, if possible.
	 * 
	 * @param target
	 *            A String of the target you want to recive the message.
	 * @param message
	 *            A String of the message you want to send.
	 */
	public void sendMessage(String target, String message) {
		AccessController.checkPermission(new ChoobPermission(
				"message.send.privmsg"));
	...
	/**
	 * Sends an action to the target you specify, may be a #channel. It is
	 * better to use sendContextReply, if possible.
	 * 
	 * @param target
	 *            A String of the target you want to recive the message.
	 ...
	 * 
	 * @param line
	 *            The line of text to send. Should not be terminated with \n.
	 */
	public void sendRawLine(String line) {
		AccessController
				.checkPermission(new ChoobPermission("message.send.raw"));
		bot.sendRawLineViaQueue(line);
	}

	public void join(String channel) throws ChoobException {
		AccessController.checkPermission(new ChoobPermission("state.join."
				+ channel));
		bot.joinChannel(channel);
	}

	public void part(String channel) throws ChoobException {
		AccessController.checkPermission(new ChoobPermission("state.part."
				+ channel));
