...
public abstract class ChoobPluginManager {
	...
	protected abstract Object createPlugin(String pluginName, URL fromLocation)
	 ...
	 * @param fromLocation
	 *            URL from which to get the plugin's contents.
	 * @return true if the plugin was reloaded
	 * @throws Exception
	 *             Thrown if there's a syntactical error in the plugin's source.
	 */
	public final boolean loadPlugin(String pluginName, URL fromLocation)
			throws ChoobException {
		AccessController.checkPermission(new ChoobPermission("plugin.load."
	...
	public final void unloadPlugin(String pluginName)
			throws ChoobNoSuchPluginException {
		AccessController.checkPermission(new ChoobPermission("plugin.unload."
				+ pluginName.toLowerCase()));
	...
	public final void addCommand(String pluginName, String commandName) {
		synchronized (phoneticCommands) {
			if (pluginName != null)
				phoneticCommands.addWord((pluginName + "." + commandName)
						.toLowerCase());
			else
	...
	public final void removeCommand(String pluginName, String commandName) {
		synchronized (phoneticCommands) {
			if (pluginName != null)
				phoneticCommands.removeWord((pluginName + "." + commandName)
						.toLowerCase());
			else
				phoneticCommands.removeWord(commandName.toLowerCase());
		}
	...
	/**
	 * Add a command to the internal database. Use the two parameter version in
	 * preference to this!
	 */
	 ...
	 *            Command to call.
	 * @param ev
	 *            Message object from IRC.
	 */
	abstract public ChoobTask commandTask(String plugin, String command,
			Message ev);
	 ...
	 *             when the call didn't exist.
	 * @throws ChoobInvocationError
	 *             when the call threw an exception.
	 */
	abstract public Object doAPI(String pluginName, String APIName,
	 ...
	 *             when the call threw an exception.
	 */
	abstract public Object doGeneric(String pluginName, String prefix,
