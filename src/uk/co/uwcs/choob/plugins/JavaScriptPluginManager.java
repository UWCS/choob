...
/*
 * Deals with all the magic for JavaScript plugins. Woo. @author silver
 */
public final class JavaScriptPluginManager extends ChoobPluginManager {
	...
	protected Object createPlugin(String pluginName, URL fromLocation)
		...
		try {
			...
			throw new ChoobException(
					"Unable to open a connection to the source location <"
							+ fromLocation + ">.");
		}
		try {
			/*
			...
			e.printStackTrace();
			throw new ChoobException("Unable to fetch the source from <"
					+ fromLocation + ">.");
		}

		// Create the new plugin instance.
		JavaScriptPlugin plug = new JavaScriptPlugin(this, pluginName, code,
				mods, irc);
	...
	public ChoobTask commandTask(String pluginName, String command, Message ev) {
		...
		JavaScriptPluginMethod method = pluginMap.getCommand(pluginName + "."
				+ command);
		if (method != null) {
	...
	public List<ChoobTask> eventTasks(Event ev) {
		...
		List<JavaScriptPluginMethod> methods = pluginMap.getEvent(ev
				.getMethodName());
		if (methods != null) {
	...
	public List<ChoobTask> filterTasks(Message ev) {
		...
		List<JavaScriptPluginMethod> methods = pluginMap.getFilter(ev
				.getMessage());
		if (methods != null) {
			for (JavaScriptPluginMethod method : methods) {
	...
	public Object doGeneric(String pluginName, String prefix,
			String genericName, Object... params)
	...
	public Object doAPI(String pluginName, String APIName,
			final Object... params) throws ChoobNoSuchCallException {
	...
	/*
	 * This calls any export a plugin has, be it a method or a property, and can
	 * return one of two things: the result, or a ChoobTask that when run does
	 * the call and gives the result.
	 * 
	 ...
	 */
	private Object callMethod(final JavaScriptPluginExport export,
			final Object[] params, final int result) {
		...
		final AccessControlContext accessContext = new AccessControlContext(
				domain);
			...
			public Object run() throws Exception {
				...
				try {
					...
					if (export instanceof JavaScriptPluginMethod) {
						...
						return JSUtils.mapJSToJava(function.call(cx, scope,
								inst, params));
					}
					...
					throw new ChoobError("Unknown export type for "
							+ export.getName() + ".");

				} catch (RhinoException e) {
					if ((params.length > 0) && (params[0] instanceof Message)) {
						irc.sendContextReply((Message) params[0], e.details()
								+ " Line " + e.lineNumber() + ", col "
								+ e.columnNumber() + " of " + e.sourceName()
								+ ".");
					} else {
						System.err.println("Exception calling export "
								+ export.getName() + ":");
					}
					e.printStackTrace();
					throw e;
					...
					if ((params.length > 0) && (params[0] instanceof Message)) {
						mods.plugin.exceptionReply((Message) params[0], e,
								pluginName);
					} else {
						System.err.println("Exception calling export "
								+ export.getName() + ":");
...
final class JavaScriptPluginMap {
	 ...
	 * 
	 * TYPE NAME IN PLUGIN RELATION NAME IN MAP Command commandFoo one
	 * pluginname.foo Event onFoo many onfoo Filter filterFoo ? /regexp/ Generic
	 * otherFooBar one pluginname.other:foobar Interval interval one pluginname
	 */
	...
	synchronized void loadPluginMap(String pluginName,
			JavaScriptPlugin pluginObj) {
				...
				if (prop instanceof String) {
					...
					if (propString.startsWith("command")) {
						...
						if (!(propVal instanceof Function)) {
							System.err
									.println("  WARNING: Command-like property that is not a function: "
											+ propString);
							continue;
						}
						// It's a function, yay!
						Function func = (Function) propVal;
						JavaScriptPluginMethod method = new JavaScriptPluginMethod(
								pluginObj, propString, func);

						String commandName = lname + "."
								+ propString.substring(7).toLowerCase();
						...
						if (helpVal instanceof Function) {
							...
							helpExport = new JavaScriptPluginMethod(pluginObj,
									propString + ".help", helpFunc);
						} else {
							helpExport = new JavaScriptPluginProperty(
									pluginObj, propString + ".help");
						}

						String fullName = lname + ".help:"
								+ propString.toLowerCase();
						...
						if (!(propVal instanceof Function)) {
							System.err
									.println("  WARNING: Event-like property that is not a function: "
											+ propString);
							continue;
						}
						// It's a function, yay!
						...
						if (events.get(eventName) == null) {
							events.put(eventName,
									new LinkedList<JavaScriptPluginMethod>());
						}
						...
						if (!(propVal instanceof Function)) {
							System.err
									.println("  WARNING: Filter-like property that is not a function: "
											+ propString);
							continue;
						}
						// It's a function, yay!
						...
						if (regexpVal == Scriptable.NOT_FOUND) {
							System.err.println("  WARNING: Filter function ("
									+ propString
									+ ") missing 'regexp' property.");
							continue;
						}
						if (!(regexpVal instanceof NativeRegExp)) {
							System.err
									.println("  WARNING: Filter function ("
											+ propString
											+ ") property 'regexp' is not a Regular Expression: "
											+ regexpVal.getClass().getName());
							continue;
						}
						...
						if (!(propVal instanceof Function)) {
							System.err
									.println("  WARNING: Interval-like property that is not a function: "
											+ propString);
							continue;
						}
						// It's a function, yay!
						...
						if (matcher.matches()) {
							...
							if (propVal instanceof Function) {
								// It's a function, yay!
								Function func = (Function) propVal;
								export = new JavaScriptPluginMethod(pluginObj,
										propString, func);
							} else {
								export = new JavaScriptPluginProperty(
										pluginObj, propString);
							}

							String prefix = matcher.group(1);
							String gName = propString
									.substring(prefix.length()).toLowerCase();
							String fullName = lname + "." + prefix + ":"
									+ gName;
	...
	synchronized void unloadPluginMap(String pluginName) {
		...
		for (NativeRegExp filter : filtersToRemove) {
			...
			// filters.get(filter).getPlugin().getName() + ")");
			count++;
 ...
 * the actual function so it can be identified and called with the right scope
 * (in JS, the call scope must be preserved, and this is handled by the plugin
 * object here).
 */
final class JavaScriptPluginMethod extends JavaScriptPluginExport {
	private Function function;

	public JavaScriptPluginMethod(JavaScriptPlugin plugin, String name,
			Function function) {
		super(plugin, name);
...
final class JavaScriptPluginProperty extends JavaScriptPluginExport {
	...
	private static Scriptable getObjectProp(Scriptable obj, String prop)
...
final class JavaScriptPlugin {
	...
	public JavaScriptPlugin(JavaScriptPluginManager plugMan, String pluginName,
		...
		try {
			...
			try {
				int flags = ScriptableObject.READONLY
						| ScriptableObject.DONTENUM
						| ScriptableObject.PERMANENT;
				ScriptableObject.defineProperty(scope,
						"__jsplugman_pluginName", pluginName, flags);
				ScriptableObject.defineProperty(scope, "dump",
						new FunctionObject("dump",
								JavaScriptPluginManager.class.getMethod("dump",
										String.class), scope), flags);
				ScriptableObject.defineProperty(scope, "dumpln",
						new FunctionObject("dumpln",
								JavaScriptPluginManager.class.getMethod(
										"dumpln", String.class), scope), flags);
			...
			if (ctor == Scriptable.NOT_FOUND) {
				throw new ChoobException("Constructor property '" + pluginName
						+ "' for JavaScript plugin not found.");
			}
			if (!(ctor instanceof Function)) {
				throw new ChoobException("Constructor property '" + pluginName
						+ "' for JavaScript plugin is not a function.");
			}
			...
			ProtectionDomain accessDomain = mods.security
					.getProtectionDomain(pluginName);
			AccessControlContext accessContext = new AccessControlContext(
					new ProtectionDomain[] { accessDomain });
			try {
				inst = AccessController.doPrivileged(
						new PrivilegedExceptionAction<Scriptable>() {
			...
			throw new ChoobException(e.details() + " at line " + e.lineNumber()
					+ ", col " + e.columnNumber() + " of " + e.sourceName()
					+ ".");

		} catch (RhinoException e) {
			throw new ChoobException(e.details() + " Line " + e.lineNumber()
					+ ", col " + e.columnNumber() + " of " + e.sourceName()
					+ ".");
