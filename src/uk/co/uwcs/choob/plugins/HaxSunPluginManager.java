...
public final class HaxSunPluginManager extends ChoobPluginManager {
	...
	public HaxSunPluginManager(Modules mods, IRCInterface irc)
		...
		try {
			...
			this.compileMethod = javac.getMethod("compile", String[].class,
					PrintWriter.class);
	...
	private URL getToolsPath() {
		...
		// If JRE
		String toolsString = libPath + fSep + ".." + fSep + "lib" + fSep
				+ "tools.jar";
		...
		if (!toolsFile.exists()) {
			...
			if (!toolsFile.exists()) {
				...
				throw new RuntimeException(
						"Choob must currently be run by a working JDK (not just JRE).");
			}
		}
		try {
			...
			throw new RuntimeException(
					"Internal error: Cannot find URL for tools.jar: " + e);
		}
	}

	private String compile(final String[] fileNames, final String classPath)
			throws ChoobException {
					...
					.doPrivileged(new PrivilegedExceptionAction<Integer>() {
						public Integer run() throws ChoobException {
							try {
								...
								return (Integer) compileMethod.invoke(null,
										newNames, output);
							} catch (IllegalAccessException e) {
								throw new ChoobException(
										"Could not invoke compiler method: "
												+ e);
		...
		if (ret == 0)
			...
			try {
				String url = (String) mods.plugin.callAPI("Http",
						"StoreString", baosts);
	...
	private String[] makeJavaFiles(String pluginName, String outDir,
	...
	protected Object createPlugin(String pluginName, URL source)
			throws ChoobException {
		String classPath = prefix + File.separator + pluginName
				+ File.separator;
		if (source != null) {
			...
			if (sourceConn.getLastModified() > javaFile.lastModified()
					|| sourceConn.getLastModified() == 0) {
				// needs updating
				InputStream in = null;
				...
				try {
					...
					throw new ChoobException("Failed to set up for compiler: "
							+ e);
				} finally {
					try {
						if (in != null)
		...
		ClassLoader loader = new HaxSunPluginClassLoader(pluginName, classPath,
				getProtectionDomain(pluginName));
		try {
			return instantiatePlugin(loader.loadClass("plugins." + pluginName
					+ "." + pluginName), pluginName);
		} catch (ClassNotFoundException e) {
			throw new ChoobException("Could not find plugin class for "
					+ pluginName + ": " + e);
		}
	}

	protected Object instantiatePlugin(Class<?> newClass, String pluginName)
			throws ChoobException {
			...
			try {
				...
				for (int j = 0; j < t.length; j++)
					if (t[j] == IRCInterface.class)
						arg[j] = irc;
					else if (t[j] == Modules.class)
						arg[j] = mods;
					else
						throw new ChoobException(
								"Unknown parameter in constructor.");
				...
				throw new ChoobException(
						"Plugin "
								+ pluginName
								+ " had no constructor (this error shouldn't occour, something serious is wrong): "
								+ e);
			} catch (InvocationTargetException e) {
				throw new ChoobException("Plugin " + pluginName
						+ "'s constructor threw an exception: " + e.getCause(),
						e.getCause());
		...
		try {
			...
			// This isn't.
			throw new ChoobException("Plugin " + pluginName
					+ "'s create() threw an exception: " + e.getCause(), e
					.getCause());
		}
	 ...
	 * @param args
	 *            The arguments of the method we hope to resolve.
	 */
			...
			public void run() {
				try {
					...
					if (param instanceof Message) {
						...
						mods.plugin.exceptionReply((Message) param, cause,
								pluginName);
					}
	...
	public Object doGeneric(String pluginName, String prefix, String genName,
		...
		try {
			...
			if (e instanceof InvocationTargetException) {
				...
				throw new ChoobError("Could not access method " + fullName
						+ ": " + e);
			else
				throw new ChoobError("Unknown error accessing method "
						+ fullName + ": " + e);
		}
	}
...
final class ChoobPluginMap {
	...
	private final Map<String, Member> genCallSigs; // plugin.prefix:genericname(params)

	// -> method
	private final Map<String, List<Member>> genCalls; // plugin.prefix:genericname

	// -> list of possible
	// methods
	private final Map<Pattern, List<Method>> filters; // pattern object ->
	...
	// Wipe out details for plugin <name>, and if pluginObj is not null, add new
	// ones.
	synchronized void resetPlugin(String pluginName, Object pluginObj) {
		...
		if (plugins.get(lname) != null) {
			...
			/*
			 * it = pluginApiCalls.get(lname).iterator(); while (it.hasNext())
			 * apiCalls.remove(it.next()); it =
			 * pluginApiCallSigs.get(lname).iterator(); while (it.hasNext())
			...
			while (it3.hasNext()) {
				...
				while (it2.hasNext()) {
					if (it2.next().getDeclaringClass().getSimpleName()
							.compareToIgnoreCase(pluginName) == 0)
						it2.remove();
				}
			}
		...
		for (Method meth : meths) {
			...
			/*
			 * else if (name.startsWith("api")) { String apiName = lname + "." +
			 * name.substring(3).toLowerCase(); if
			 * (HaxSunPluginManager.checkAPISignature(meth)) {
			 * apis.add(apiName); if (apiCalls.get(apiName) == null)
			 * apiCalls.put(apiName, new LinkedList<Method>());
			 * apiCalls.get(apiName).add(meth); } else { System.err.println("API
			 * call " + apiName + " had invalid signature."); } } API == generic
			 */
			else if (name.startsWith("filter")) {
				String filter;
				try {
					filter = (String) pluginClass.getField(name + "Regex").get(
							pluginObj);
				} catch (NoSuchFieldException e) {
					System.err.println("Plugin " + pluginName + " had filter "
							+ name + " with no regex.");
					continue;
				} catch (ClassCastException e) {
					System.err.println("Plugin " + pluginName + " had filter "
							+ name + " with no non-String regex.");
					continue;
				} catch (IllegalAccessException e) {
					System.err.println("Plugin " + pluginName
							+ " had non-public filter " + name + ".");
					continue;
				}
				Pattern pattern;
				try {
				...
				if (HaxSunPluginManager.checkCommandSignature(meth)) {
					...
					System.err.println("Filter " + lname + "." + name
							+ " had invalid signature.");
				}
				...
				if (HaxSunPluginManager.checkEventSignature(meth)) {
					...
					System.err.println("Event " + lname + "." + name
							+ " had invalid signature.");
				}
				...
				if (HaxSunPluginManager.checkIntervalSignature(meth)) {
					...
					System.err.println("Interval " + lname + "." + name
							+ " had invalid signature.");
				}
			} else {
				...
				if (matcher.matches()) {
					...
					String gName = name.substring(prefix.length())
							.toLowerCase();
					...
					if (HaxSunPluginManager.checkAPISignature(meth)) {
						...
						System.err.println("Generic call " + fullName
								+ " had invalid signature.");
					}
				} else {
		...
		for (Field field : fields) {
			...
			/*
			 * else if (name.startsWith("api")) { } API == generic
			 */
			else if (name.startsWith("filter")) {
			} else if (name.startsWith("on")) {
			} else if (name.startsWith("interval")) {
	...
	synchronized Object getPluginObj(Member meth) {
		return getPluginObj(meth.getDeclaringClass().getSimpleName()
				.toLowerCase());
	}
	...
	/*
	 * synchronized Method getAPI(String apiName) { return
	 * apiCallSigs.get(apiName.toLowerCase()); }
	 * 
	 ...
	 * 
	 * synchronized List<Method> getAllAPI(String apiName) { return
	 * apiCalls.get(apiName.toLowerCase()); }
	 */
	...
	synchronized void setGeneric(String genName, Member obj) {
		pluginGenCallSigs.get(
				obj.getDeclaringClass().getSimpleName().toLowerCase()).add(
				genName.toLowerCase());
