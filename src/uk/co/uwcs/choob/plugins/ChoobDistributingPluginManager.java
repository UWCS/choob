...
public final class ChoobDistributingPluginManager extends ChoobPluginManager {
	 ...
	 */
	public ChoobTask commandTask(final String plugin, final String command,
			final Message ev) {
		...
		if (task == null) {
			...
			task = new ChoobTask(null, "commandTask:suggestions") {
				public void run() {
					String[] commands;
					final String notFoundPrefix = "Command " + plugin + "."
							+ command + " not found";
					try {
						...
						if (commands.length == 0)
							irc
									.sendContextReply(
											ev,
											notFoundPrefix
													+ ", the plugin doesn't have any commands in it!");
						else if (commands.length == 1)
							irc.sendContextReply(ev, notFoundPrefix
									+ ", you must have meant " + plugin + "."
									+ commands[0] + "?");
						else {
							StringBuilder sb = new StringBuilder(notFoundPrefix)
									.append(", did you mean one of ")
									// Plugin's/pluginnames'
									.append(plugin).append("'").append(
											plugin.substring(
													plugin.length() - 1,
													plugin.length())
													.equalsIgnoreCase("s") ? ""
							...
							irc.sendContextReply(ev, sb.append(
									commands[commands.length - 1]).append("?")
									.toString());
						}
						...
						irc
								.sendContextReply(
										ev,
										notFoundPrefix
												+ ", the plugin doesn't exist or isn't loaded.");
					}
				}
			};
		}
	 ...
	 */
	public Object doAPI(String pluginName, String APIName, Object... params)
	 ...
	 * 
	 * @param prefix
	 *            The prefix (ie call type) of the call.
	 * @param genericName
	...
	public Object doGeneric(String pluginName, String prefix,
		...
		throw new ChoobNoSuchPluginException(pluginName, "generic: " + prefix
				+ ":" + genericName);
	}
}
