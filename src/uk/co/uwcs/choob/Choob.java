...
public final class Choob extends PircBot {
	...
	Choob() throws ChoobError {
		...
		try {
			broker = new DbConnectionBroker(
					...
					"jdbc:mysql://"
							...
							+ "/"
							+ conf.getSettingFallback("database", "choob")
							+ "?autoReconnect=true&"
							+ "autoReconnectForPools=true&initialTimeout=1&"
							+ "useUnicode=true&characterEncoding=UTF-8&characterSetResults=UTF-8",
					conf.getSettingFallback("dbUser", "choob"), conf
							.getSettingFallback("dbPass", ""), 10, 20, logFile,
					60);
		} catch (SQLException e) {
			e.printStackTrace();
			System.out
					.println("Unexpected error in DbConnectionBroker setup, exiting.");
			System.exit(5);
			return;
		}
		...
		try {
			...
			e.printStackTrace();
			System.out
					.println("Unhandled IRC Error on connect, exiting: ." + e);
			System.exit(3);
		}
	}

	// * Connect the initialised bot to IRC, and do hard-coded
	// post-connection stuff. */
	void doConnect() throws IOException, IrcException {
		exitCode = -1;

		// Connect to the IRC server.
		connect(conf.getSettingFallback("server", "irc.uwcs.co.uk"), Integer
				.parseInt(conf.getSettingFallback("port", "6667")), conf
				.getSettingFallback("password", null));
	...
	/**
	 * Return a regex that can (and should) be used to match lines for a prefix
	 * indicating that the line is a command.
	 */
	public String getTriggerRegex() {
		return "^(?i:" + trigger + "|"
				+ getName().replaceAll("([^a-zA-Z0-9-])", "\\\\$1")
				+ "[,:]\\s*" + "|bot[,:]\\s*)";
	}

	/**
	 * Return a string which can (and should) be prepended onto a line to make
	 * it into a command.
	 */
	 ...
	 * 
	 * This code is automatically replaced by HorriblePerlScript when it is run.
	...
	protected void onMessage(String target, String nick, String login,
		...
		spinThread(new ChannelMessage("onMessage", System.currentTimeMillis(),
				((int) (Math.random() * 127)), message, nick, login, hostname,
				target, target));
	}
	...
	protected void onPrivateMessage(String nick, String login, String hostname,
		...
		spinThread(new PrivateMessage("onPrivateMessage", System
				.currentTimeMillis(), ((int) (Math.random() * 127)), message,
				nick, login, hostname, null));
	}
	...
	protected void onChannelInfo(String channel, int userCount, String message) {
		spinThread(new ChannelInfo("onChannelInfo", System.currentTimeMillis(),
				((int) (Math.random() * 127)), message, channel));
	}
	...
	protected void onDeVoice(String channel, String nick, String login,
		...
		spinThread(new ChannelUserMode("onDeVoice", System.currentTimeMillis(),
				((int) (Math.random() * 127)), channel, "v", false, target));
	}
	...
	protected void onDeop(String channel, String nick, String login,
		...
		spinThread(new ChannelUserMode("onDeop", System.currentTimeMillis(),
				((int) (Math.random() * 127)), channel, "o", false, target));
	}
	...
	protected void onInvite(String target, String nick, String login,
		...
		spinThread(new ChannelInvite("onInvite", System.currentTimeMillis(),
				((int) (Math.random() * 127)), channel, nick, login, hostname,
				target));
	}
	...
	protected void onKick(String channel, String nick, String login,
		...
		spinThread(new ChannelKick("onKick", System.currentTimeMillis(),
				((int) (Math.random() * 127)), message, channel, nick, login,
				hostname, target));
	}
	...
	protected void onMode(String channel, String nick, String login,
		...
		spinThread(new ChannelModes("onMode", System.currentTimeMillis(),
				((int) (Math.random() * 127)), channel, modes));
	}

	protected void onNickChange(String nick, String login, String hostname,
			String newNick) {
	...
	protected void onQuit(String nick, String login, String hostname,
			String message) {
		spinThread(new QuitEvent("onQuit", System.currentTimeMillis(),
				((int) (Math.random() * 127)), message, nick, login, hostname));
	}
	...
	protected void onRemoveChannelBan(String channel, String nick,
		...
		spinThread(new ChannelParamMode("onRemoveChannelBan", System
				.currentTimeMillis(), ((int) (Math.random() * 127)), channel,
				"b", false, param));
	}
	...
	protected void onSetPrivate(String channel, String nick, String login,
		...
		spinThread(new ChannelMode("onSetPrivate", System.currentTimeMillis(),
				((int) (Math.random() * 127)), channel, "p", true));
	}
	...
	protected void onSetSecret(String channel, String nick, String login,
		...
		spinThread(new ChannelMode("onSetSecret", System.currentTimeMillis(),
				((int) (Math.random() * 127)), channel, "s", true));
	}
	...
	protected void onServerResponse(int code, String response) {
		spinThread(new ServerResponse("onServerResponse", System
				.currentTimeMillis(), ((int) (Math.random() * 127)), code,
				response));
	}
	...
	protected void onVoice(String channel, String nick, String login,
		...
		spinThread(new ChannelUserMode("onVoice", System.currentTimeMillis(),
				((int) (Math.random() * 127)), channel, "v", true, target));
	}
