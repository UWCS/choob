...
public final class ChoobThreadManager extends ThreadPoolExecutor {
	...
	private static final long PLUGIN_WAIT_OBJECT_SETUP_TIME = 1 * 60 * 1000; // 1

	// minute
	private static final long PLUGIN_WAIT_OBJECT_EXIRES = 15 * 60 * 1000; // 15
		...
		if (next != null) {
			...
			synchronized (waitObjects) {
				...
				if ((waitObject.sem.availablePermits() == waitObject.limit)
						&& ((waitObject.expires < System.currentTimeMillis()) || (this.pwoSetupTime > System
								.currentTimeMillis()))) {
	...
	protected void beforeExecute(Thread thread, Runnable task) {
		...
		// runningTasks.put((ChoobTask)task, (pluginName == null ? "S:" + sf :
		// "P:" + pluginName + (sf != null ? ":S:" + sf : "")));
		// }
	...
	public static void queueTask(ChoobTask task)
			throws RejectedExecutionException {
		java.security.AccessController.checkPermission(new ChoobPermission(
				"task.queue"));
		exe.queue(task);
	}
	...
	private void queue(ChoobTask task) throws RejectedExecutionException {
		// synchronized(runningTasks) {
		// System.out.print("ChoobThreadManager.beforeExecute: pool usage = " +
		...
		if (acquireOK) {
			...
			if (!queue.offer(task))
				...
				throw new RejectedExecutionException("Plugin " + pluginName
						+ " has too many queued tasks!");
		}
	}
}
