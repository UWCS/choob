...
public final class HistoryModule {
	 ...
	 * 
	 * @param ev
	 *            {@link Event} object representing the line from IRC.
	 * @throws Exception
	 *             Thrown from the database access, potential SQL or IO
	 *             exceptions.
	 */
	...
	public void addLog(Event ev) {
		...
		try {
			...
			if (ev instanceof Message) {
				...
				insertLine.setString(3, mes.getLogin() + "@"
						+ mes.getHostname());
			...
			System.err
					.println("Could not write history line to database: " + e);
			// I think this exception doesn't need to be propogated...
			// --bucko
		} finally {
			try {
				if (insertLine != null)
	...
	public int getMessageID(Message mes) {
		...
		try {
			...
			System.err.println("Could not read history line from database: "
					+ e);
			...
			try {
				...
				System.err
						.println("Could not read history line from database: "
								+ e);
				throw new ChoobError("SQL Error reading from database.");
			} finally {
	 ...
	 * 
	 * @param messageID
	 *            The message ID, as returned from getMessageID.
	...
	public Message getMessage(int messageID) {
		...
		try {
			dbCon = dbBroker.getConnection();
			stat = dbCon
					.prepareStatement("SELECT * FROM History WHERE LineID = ?");
	...
	/**
	 * Get as the most recent Message from the history of the channel in which
	 * cause occurred.
	 * 
	 * @param cause
	 *            The "cause" - only messages that occurred before this are
	 *            processed
	...
	public Message getLastMessage(Message cause) {
		...
		throw new IllegalArgumentException(
				"Message passed to getLastMessage was not a ChannelEvent.");
	}
	...
	/**
	 * Get as many as possible up to count Messages from the history of the
	 * channel in which cause occurred.
	 * 
	 * @param channel
	 *            The channel to read
	 * @param count
	 *            The maximal number of messages to return.
	 * @return A list of message objects, the first being the most recent.
	 */
	...
	public List<Message> getLastMessages(Message cause, int count) {
		if (cause instanceof ChannelEvent)
			return getLastMessages(((ChannelEvent) cause).getChannel(), cause,
					count);
		throw new IllegalArgumentException(
				"Message passed to getLastMessages was not a ChannelEvent.");
	}
	 ...
	 * @return A list of message objects, the first being the most recent.
	 */
	public List<Message> getLastMessages(final String channel, Message cause,
			int count) {
		Connection dbCon = null;
		PreparedStatement stat = null;
		try {
			dbCon = dbBroker.getConnection();
			stat = dbCon
					.prepareStatement("SELECT * FROM History WHERE Channel = ? AND Time < ? AND Time > ? - 60*60*12*1000 ORDER BY Time DESC, LineID DESC LIMIT ?");
			stat.setString(1, channel);
			stat.setLong(2, cause == null ? System.currentTimeMillis() : cause
					.getMillis());
			stat.setLong(3, cause == null ? System.currentTimeMillis() : cause
					.getMillis());
			stat.setInt(4, count);
			...
			if (result.first()) {
				do {
					...
					/*
					 * Sanity check; since we use <, this shouldn't happen... if
					 * (mes.equals(cause)) { System.out.println("Skipping
					 * event..."); continue; }//
					 */
