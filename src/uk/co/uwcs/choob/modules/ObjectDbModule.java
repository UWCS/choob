 ...
 * <p>
 * 
 * All public read/write properties of the following types are loaded and saved
 * by ObjectDB for Java objects:
 * 
 * <ul>
 * <li>Boolean</li>
 * <li>Byte</li>
 ...
 * saves ones that do not start with an underscore (<tt>_</tt>). The
 * following JavaScript types are saved:
 * 
 * <ul>
 * <li>Boolean</li>
 * <li>Number</li>
...
public final class ObjectDbModule {
	 ...
	 * 
	 * @param storedClass
	 *            The .class of the object you want to retrieve.
	 * @param clause
	 *            The clause specifying which objects you want to select.
	 */
	 ...
	 * Connection dbConn = null; try { dbConn = broker.getConnection(); } catch
	 * (SQLException e) { throw new ChoobError("Sql Exception", e); }
	 * ObjectDBTransaction trans = new ObjectDBTransaction();
	 * trans.setConn(dbConn); trans.setMods(mods); try { return
	 * trans.retrieveInt(storedClass, clause); } finally {
	 * broker.freeConnection( dbConn ); } }
	 * 
	 */
	...
	public void delete(final Object strObject) {
		synchronized (strObject.getClass()) {
			runTransaction(new ObjectDBTransaction() {
				public void run() {
					delete(strObject);
				}
			});
	...
	public void runTransaction(ObjectDBTransaction trans) {
		...
		trans.setMods(mods);
		try {
			// Attempt up to 20 backoffs with initial delay 100ms and
			// exponent 1.3.
			int delay = 100;
			for (int i = 0; i < 21; i++) {
				try {
					...
					trans.rollback();
					System.err
							.println("SQL Deadlock occurred. Rolling back and backing off for "
									+ delay + "ms.");
					try {
						synchronized (trans) {
							trans.wait(delay);
