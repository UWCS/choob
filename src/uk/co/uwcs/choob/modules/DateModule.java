...
public final class DateModule {
	 ...
	 * 0), days (index 1), hours, etc.
	 */
	final static long[] getSt(long i) {
		final long w = (i / (7 * 24 * 60 * 60 * 1000));
		i -= w * (7 * 24 * 60 * 60 * 1000);
		final long d = (i / (24 * 60 * 60 * 1000));
		i -= d * (24 * 60 * 60 * 1000);
		final long h = (i / (60 * 60 * 1000));
		i -= h * (60 * 60 * 1000);
		final long m = (i / (60 * 1000));
		i -= m * (60 * 1000);
		final long s = (i / (1000));
		i -= s * 1000;
		final long ms = (i);
	 ...
	 *            Minimum level of output, ie. passing 'days' here will cause
	 *            1w2d3h4m5s to only output 1w2d. Default is 'millisecond'.
	 */
	// FIXME: make replyDetail final.
	public final String timeStamp(final long thing, final boolean shortTokens,
			int replyDetail, final longtokens minGranularity) {
		...
		// Go through the "st", discard empty or invalid parts until we have
		// enough (replyDetail).
		...
		if (use.length == 0)
			...
			for (int i = 0; i < use.length; i++) {
				...
				// it (and any padding required) to the return value.
