...
public final class IntervalModule {
	...
	/**
	 * Calls the 'interval' function in the calling plugin, with the specified
	 * parameter and after the specified interval.
	 * 
	 * @param parameter
	 *            The paramater that you want passed along to the interval
	 *            function.
	 * @param interval
	 *            The Date at which you want the event to occour.
	 */
	 ...
	 *            The Date at which you want the event to occour.
	 * @param id
	 *            The unique id of this interval (-1 for no ID).
	 */
	 ...
	 *            function.
	 * @param delay
	 *            The delay after which you want the event to occour.
	 */
	...
	private void callBackReal(Object parameter, long when, int id) {
		...
		if (plugin == null) {
			System.err
					.println("A plugin tried to call callBack, but wasn't on the stack...");
			return;
		}
		...
		synchronized (intervalList) {
			if (id != -1) {
				...
				while (iterator.hasNext()) {
					...
					if (id == thisInt.getId()
							&& plugin.equals(thisInt.getPlugin()))
						iterator.remove();
				}
			}
	...
	public void reset() {
		...
		if (plugin == null) {
			System.err
					.println("A plugin tried to call reset, but wasn't on the stack...");
			return;
		}
		synchronized (intervalList) {
			// XXX this is damn inefficient...
