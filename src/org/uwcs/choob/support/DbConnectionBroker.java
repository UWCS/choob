...
public class DbConnectionBroker implements Runnable {
	 ...
	 */
	public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns, int maxConns, String logFileString, double maxConnTime, boolean logAppend)
	throws IOException
	{
	...
	private void setupBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns, int maxConns, String logFileString, double maxConnTime, boolean logAppend, int maxCheckoutSeconds, int debugLevel) throws IOException
	{
		...
		// System.out.println("[DbConnectionBroker:setBroker] Called..");
		...
		try {
			// System.out.println("[DbConnectionBroker:setBroker] Creating log..");
			log = new PrintWriter(new FileOutputStream(logFileString, logAppend), true);
			// System.out.println("[DbConnectionBroker:setBroker] Finished creating log..");
			// Can't open the requested file. Open the default file.
		} catch (IOException e1) {
			try {
				log = new PrintWriter(new FileOutputStream("/tmp/DbConnections.log", logAppend),true);
		...
		// Write the file (used to clean up dead/broken connection)
		...
		try { pidout = new BufferedWriter(new FileWriter(logFileString + "pid")); }
		catch (IOException e) {
		...
		// System.out.println("[DbConnectionBroker:setBroker] Writing to log (" + logFileString + ")..");
		...
		// System.out.println("[DbConnectionBroker:setBroker] Written to log (" + logFileString + ")..");
		...
		// System.out.println("[DbConnectionBroker:setBroker] Attempting to initialise pool.");
			...
			for(int i=1; i < dbLoop; i++) {
				try
				{
					for(int j=0; j < currConnections; j++) {
						// System.out.println("[DbConnectionBroker:setBroker] Creating conneciton " + j);
						createConn(j);
					}
					connectionsSucceeded=true;
					// System.out.println("[DbConnectionBroker:setBroker] Created all connections successfully..");
					break;
				} catch (SQLException e)
				{
					System.out.println("SQL Error, see log for details.");
					if(debugLevel > 0)
					{
						log.println("--->Attempt (" + String.valueOf(i) + " of " + String.valueOf(dbLoop) + ") failed to create new connections set at startup: ");
						log.println("    " + e);
	...
	long maxCheckoutMillis = maxCheckoutSeconds * 1000;
		...
		while(forever) {
			...
			try {
				BufferedReader in;
				try { in = new BufferedReader(new FileReader(logFileString + "pid")); }
				catch (IOException e) {
					in = new BufferedReader(new FileReader("." + logFileString + "pid"));
				}
	...
	public boolean isValidated( Connection tempCon )
	{
		try
		{
			...
			{
				log.println("Error: Connection failed validation.");
				tempCon.close();
				return false;
			}
		}
